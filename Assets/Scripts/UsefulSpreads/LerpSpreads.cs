...
// this is the updated one methinks
...
public class TimeLerp<T>
{
    ...
    public (T position, float progress) Go()
    {
        /*if (!_running)
        {
            _startTime = Time.time - (_progress * _speed);
            _running = true;
        }*/

        // first lerp the progress, this should stay linear

        //_progress = Mathf.Lerp(0, 1,  (Time.time - _startTime) * _speed);
...
public class MultiTimeLerp<T>
{
    ...
    public (T position, float progress) Go()
    {
        if (!_startOfInternalLerp)
        {
            _startTime = Time.time - ((_overallProgress * _speed));
            _startOfInternalLerp = true;
        }

        // first lerp the progress, this should stay linear
        _overallProgress = Mathf.Lerp(0, 1, ((Time.time - _startTime) * _speed));
